...
class CinemaHall:
    ...
    def print_cinema_hall(self):
        for row in range(self.seating_plan):
    ...
    def book_ticket(self):
        rows = ['A', 'B', 'C', 'D', 'E']
        rows = input("enter row(A-E): ")
        idx = -1
        email = input("enter sur email: ")
        for i in range(len(rows)):
            if row  == rows[i]:
                idx = i
                break
        seat_number = int(input("enter seat number[1-5]: "))

        self.seating_plan[idx][seat_number-1].is_booked = True
        self.seating_plan[idx][seat_number - 1].user_email = email

        print("thank u fr booking..............")
